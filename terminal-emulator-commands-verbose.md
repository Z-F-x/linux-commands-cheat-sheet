...
ls: This is the basic command used to list the contents of a directory.

-1: This option tells ls to list the directory contents one per line. Without this option, ls may display the files in a more compact format (e.g., multiple files per row).

-d: This option modifies ls to list the directories themselves, rather than the contents of the directories. Normally, ls will show both files and directories within the specified directory, but -d ensures only the directory itself is listed.

*/: This is a pattern that matches only directories. The asterisk * means "any number of any characters," and the trailing slash / ensures only directories (which end with /) are listed, excluding files.
